...
float blanco[]={1,1,1};
float negro[]={0,0,0},azul[]={0,0,1}, verdeoscuro[]={0,0.6,0};
...
public:
 ...
 glColor3fv(cyan);
      glBegin(GL_POLYGON);
         glVertex3f(0,0,1);
         for (float i = 0; i <= (2*PI)+0.1f; i += 0.1f )
            glVertex2d(50*sqrt(cos(2*i))*sin(i+PI),50*sqrt(cos(2*i))*cos(i+PI));
      glEnd();
 ...
 glColor3fv(cyan);
      glBegin(GL_POLYGON);
         glVertex3f(0,0,1);
         for (float i = 0; i <= (2*PI)+0.1f; i += 0.1f )
            glVertex2d(50*sqrt(cos(2*i))*cos(i),50*sqrt(cos(2*i))*sin(i));
      glEnd();
   ...
   void Iniciar() {
      pos[0] = 400; //INICIAMOS EN LA POSICION 400 EN EL EJE X
      pos[1] = 300; // E 300 EN EL EJE Y
      pos[2] = tiempo = toques = 0; // INICIAMOS POS (2) , TIMER Y TOQUES EN CERO
      vel[0] = vel[1] = 20; // VEL(0) Y VEL(1) EN 25
...
void PintarEscena() {
   ...
   glRectf(0,0,800,600); //aqui se define el tamaño del cuadro de la ventana
    ...
    cuadrilatero(b,600,800,3,verdeoscuro);
    cuadrilatero(c,800,10,3,blanco);
    cuadrilatero(b,10,800,3,blanco);
    cuadrilatero(b,600,10,3,blanco);
    cuadrilatero(e,10,800,3,blanco);
    cuadrilatero(f,600,10,3,blanco);
    circunferencia(90,400,300,3,5,blanco);
    cuadrilatero(g,10,80,3,blanco);
    cuadrilatero(h,10,80,3,blanco);
    cuadrilatero(i,110,10,3,blanco);
    cuadrilatero(j,10,80,3,blanco);
    cuadrilatero(k,10,80,3,blanco);
    cuadrilatero(j,110,10,3,blanco);
    cuadrilatero(g,110,10,3,negro);
    cuadrilatero(l,110,10,3,negro);
...
int main(int argc, char **argv) {  //Función de entrada del programa
      ...
      cout<< "*                                                  *"<<endl;
      cout<< "*     Ayuda a la Bola Mundial a no caer            *"<<endl;
